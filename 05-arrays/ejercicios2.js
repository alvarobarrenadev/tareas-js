...
*/
let sumNumbers = doubleNumbers.reduce((suma, number) => suma + number, 0);
...
*/
let sortNumbers = [1, 2, 7, 3, 5, 6];
console.log(`Números ordenados al revés:\n${sortNumbers.sort().reverse().join(", ")}`)


/* Ejercicio 6

Dado dos arrays de números, array1 y array2, crea una función que devuelva un nuevo array que contenga los elementos que son múltiplos de 3 y no sean múltiplos de 5.

Además, asegúrate de que el nuevo array no contenga duplicados.
...
const estudiantesMayoresDeEdad = estudiantes.filter(estudiantesMayores => estudiantesMayores.edad >= 18);
...
let promediosCalificaciones = estudiantesMayoresDeEdad.map(estudiante => {
  ...
  return {
    ...estudiante,
    media: Number(suma / estudiante.calificaciones.length).toFixed(2),
  };
})
...
let mayorMedia = promediosCalificaciones.reduce((mayorNota, estudiante) => {
  if (estudiante.media > mayorNota.media) return estudiante;
  return mayorNota;
...
const ingresosTotales = ventas.map(venta => {
  return {
    ...venta,
...
const mayorVenta = ingresosTotales.reduce((ventaMayor, venta) => {
  return venta.subtotal > ventaMayor.subtotal ? venta : ventaMayor;
},ingresosTotales[0]);
